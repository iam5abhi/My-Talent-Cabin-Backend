...
exports.signup = async (req, res, next) => {
  try {
    ...
    if (base64.decode(password) !== base64.decode(confirmPassword))
    ...
    const newUser = new User({
      ...
      confirmPassword: base64.decode(confirmPassword),
    });
...
exports.login = async (req, res, next) => {
  try {
    ...
    const user = await User.findOne(
      { $and: [{ email: req.body.email }, { status: "active" }] },
      {
        createdAt: 0,
        updatedAt: 0,
        education: 0,
...
exports.getAllSubCategory = async (req, res, next) => {
  const subcategory = await SubCategory.find(
    {},
    { categoryId: 0, status: 0, createdAt: 0, updatedAt: 0 }
  );
...
exports.addLanguage = async (req, res, next) => {
  User.updateOne(
    ...
    { $addToSet: { language: { $each: req.body.language } } }
  ).exec((err, result) => {
    if (err) {
      next(new Error(`${err.message}`, 500));
    } else {
...
exports.ResendOtp = (req, res, next) => {
  ...
  User.findOne({ PhoneNumber: req.query.PhoneNumber }, function (err, data) {
    ...
    axios
      .get(
        `https://sms.innuvissolutions.com/api/mt/SendSMS?APIKey=Try50kmHFUqu0MoBnX9Ojg&senderid=EDUTEK&channel=Trans&DCS=0&flashsms=0&number=${data.PhoneNumber}&text=%20Dear%20Parent,Your%20OTP%20for%20App%20Login%20is%20${otp}%20EDUTEK&route=1014&peid=1201159350821274881`
      )
      .then((response) => {
        res.status(200).json({
